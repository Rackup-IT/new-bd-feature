...
import AdDisplay from '@/features/post-detail/components/ad-display';
...
export async function generateStaticParams() {
  ...
  try {
    const response = await fetch(`${process.env.DOMAIN}/api/v1/post?limit=20`, {
      next: { revalidate: 3600 },
    });

    if (!response.ok) return [];

    const posts = (await response.json()) as DBResponseSchema[];
...
const PostDetailPage = async (props: { params: Promise<LangParam> }) => {
  ...
  // Fetch related posts more efficiently
  const fetchRelatedPosts = async (): Promise<DBResponseSchema[]> => {
    if (!process.env.DOMAIN) return [];
    try {
      // Only fetch posts from the same section as an optimization
      const section = post.section;
      const res = await fetch(`${process.env.DOMAIN}/api/v1/post?section=${section}&limit=10`, {
        method: 'GET',
        next: { revalidate: 300 }, // Cache for 5 minutes
      });
      if (!res.ok) return [];
      const sectionPosts = (await res.json()) as DBResponseSchema[];
      ...
      const scored = sectionPosts
        ...
        .map((p) => {
          ...
          return { post: p, score: sharedKeys };
        })
      ...
      <article>
        ...
        <div className="wrapper mx-auto mt-8 lg:mt-12">
          ...
          <div className="md:hidden">
            <AuthorDetail authorData={post.writter} lang={lang} />
          </div>
          ...
          <div className="md:grid md:grid-cols-12 md:gap-6 lg:gap-8">
            ...
            <main className="md:col-span-7 lg:col-span-7">
              ...
              {/* Tags */}
              <Tags keywords={post.keywords} lang={lang} />

              {/* Subscribe area */}
              <SubscribeArea lang={lang} />
            </main>
              ...
              <div className="mb-8">
                ...
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link
                      href={`/${lang}/profile/${post.writter._id || 'unknown'}`}
