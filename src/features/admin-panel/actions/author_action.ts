...
const authorSignUpInputDataValidation = (
  data: FormData
): ClientSideValidationSchema => {
...
export const createNewAuthorAction = async (data: FormData) => {
  ...
  try {
    ...
    const findUserExistOrNot = await db
      .collection("authors")
      .findOne({ email: validData.email });

    if (findUserExistOrNot) {
      throw new Error(
        "This is user is already sign up, Please try to log-in now"
      );
    }
    ...
    const insertAuthorToDB = await db
      .collection("authors")
      .insertOne(newAuthor);

    if (!insertAuthorToDB.insertedId) {
      throw new Error(
        "Failed to create the author in database! try again later"
      );
    }
...
export const getAuthorsAction = async (filter?: {
